...
use std::env;
use std::fs::{read_to_string, OpenOptions, write};
use std::io::Write;
use std::sync::Arc;
...
async fn validate_router_quote(
    ...
    // Get reserves
    let (reserve_in, reserve_out) = get_pair_reserves(pair_address, provider, token_in, token_out)
        .await?;
    
    // Calculate expected amount_out using XYK formula
    let calculated_amount_out = calculate_xyk_amount_out(reserve_in, reserve_out, amount_in, fee_bps)?;
    ...
    let calculated_out_f64 = calculated_amount_out.as_u128() as f64 / 10f64.powi(decimals_out as i32);
...
async fn main() -> Result<()> {
    ...
    println!("Position size: {:.2} USDC.e (configure via POSITION_SIZE_USDC env var)", initial_amount_usdc);
        ...
        move |filename: &str| {
            ...
            if let Ok(content) = read_to_string(filename) {
                ...
                if entry_count > max_entries {
                    ...
                    let entries: Vec<&str> = parts.into_iter()
    ...
    let save_earnings_to_file = |opps: u64, gross: f64, gas: f64, dex_fees: f64, net: f64, checks: u64, price_diffs: &[f64], gas_costs: &[f64]| {
            ...
            .open(filename)
        {
            ...
            if opps > 0 {
                writeln!(file, "Average per trade:    ${:.2} USDC.e", net / opps as f64).ok();
            }
            writeln!(file, "").ok();
            ...
            writeln!(file, "").ok();
            if opps == 0 && checks > 0 {
                writeln!(file, "NOTE: No profitable opportunities found. This is NORMAL because:").ok();
                writeln!(file, "  - Markets are efficient (arbitrage gets exploited quickly)").ok();
                writeln!(file, "  - Gas costs require significant price differences to be profitable").ok();
        ...
        {
            writeln!(file, "╔══════════════════════════════════════════════════════════╗").ok();
            writeln!(file, "║         📊 TOTAL EARNINGS SUMMARY (SINCE START)         ║").ok();
            writeln!(file, "╠══════════════════════════════════════════════════════════╣").ok();
            ...
            if opps > 0 {
                writeln!(file, "║  Average per trade:    ${:>33.2} ║", net / opps as f64).ok();
            }
            writeln!(file, "╠══════════════════════════════════════════════════════════╣").ok();
            writeln!(file, "║  MARKET STATISTICS                                      ║").ok();
            writeln!(file, "╠══════════════════════════════════════════════════════════╣").ok();
            ...
            if !price_diffs.is_empty() {
                ...
                let max_price_diff = price_diffs.iter().fold(0.0f64, |a, &b| a.max(b));
                let min_price_diff = price_diffs.iter().fold(f64::MAX, |a, &b| a.min(b));
                writeln!(file, "║  Avg price difference:  {:>30.3}% ║", avg_price_diff).ok();
                writeln!(file, "║  Max price difference:  {:>30.3}% ║", max_price_diff).ok();
                writeln!(file, "║  Min price difference:  {:>30.3}% ║", min_price_diff).ok();
            }
            if !gas_costs.is_empty() {
                let avg_gas = gas_costs.iter().sum::<f64>() / gas_costs.len() as f64;
  ...
  "market_stats": {{
    ...
    save_earnings_to_file(total_opportunities, total_gross_profit, total_gas_cost, total_dex_fees, total_net_profit, total_checks, &total_price_diffs, &total_gas_costs);
    ...
    if max_earnings_entries > 0 {
        println!("   - earnings.txt: Detailed log (keeping last {} entries)", max_earnings_entries);
    } else {
    ...
    println!("🔍 Watching for new blocks (starting from block #{})...", last_block_number);
    ...
    loop {
        ...
        match provider.get_block_number().await {
            Ok(current_block) if current_block > last_block_number => {
                ...
                println!("[{}] Block #{} detected", Utc::now().format("%H:%M:%S"), current_block);
        ...
        let gas_cost_avax = match calculate_gas_cost(&provider, GAS_LIMIT_PER_SWAP).await {
            ...
            Err(e) => {
                println!("   Warning: Could not fetch gas price: {e}, using fallback 0.015 AVAX");
            ...
            .await
        {
            Ok(amounts) if amounts.len() >= 2 => {
                ...
                // Calculate slippage
                let slippage = calculate_slippage_joe(&joe_router, amount_in, amount_out, usdc, wavax, 6, 18)
                    .await
                    .unwrap_or(0.0);
            ...
            .await
        {
            Ok(amounts) if amounts.len() >= 2 => {
                let amount_out = amounts[1].as_u128() as f64 / 1e18; // WAVAX has 18 decimals
                // Calculate slippage
                let slippage = calculate_slippage_pangolin(&pangolin_router, amount_in, amount_out, usdc, wavax, 6, 18)
                    .await
                    .unwrap_or(0.0);
        ...
        if let (Some(joe_out), Some(pangolin_out)) = (joe_wavax_out, pangolin_wavax_out) {
            ...
            if let Some(usdc_received) = pangolin_usdc_back {
                ...
                let gas_cost_usdc = match get_avax_price_in_usdc(&provider, &joe_router, usdc, wavax).await {
                ...
                let dex_fees_usdc = calculate_dex_fees(initial_amount_usdc, joe_out, usdc_received);
                ...
                if roi_net >= MIN_ROI_THRESHOLD {
                    ...
                    println!("   Net ROI:         {roi_net_pct:.3}% (threshold: {:.1}%)", MIN_ROI_THRESHOLD * 100.0);
                    println!("   ✅ Net ROI exceeds minimum threshold ({:.1}%)", MIN_ROI_THRESHOLD * 100.0);
            ...
            if let Some(usdc_received) = joe_usdc_back {
                ...
                // Calculate DEX fees dynamically from actual swap amounts (for display only)
                let dex_fees_usdc = calculate_dex_fees(initial_amount_usdc, pangolin_out, usdc_received);
                let gross_profit_usdc = usdc_received - initial_amount_usdc;
            ...
            if !found_profit {
                ...
                let min_price_diff_pct = if (joe_out + pangolin_out) > 0.0 {
                    ...
                    if best_return > 0.0 {
                        ...
                        let slippage_factor = (initial_amount_usdc - best_return) / initial_amount_usdc;
                        // Need: min_profit_needed + current_loss
                        let total_needed = min_profit_needed + (initial_amount_usdc - best_return);
                        // Price diff needed = total_needed / (1 - slippage_factor)
                        // Then convert to percentage
                        let min_price_diff = total_needed / (1.0 - slippage_factor.max(0.01));
